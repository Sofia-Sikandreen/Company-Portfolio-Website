...
const socials = [
  ...
  { label: 'LinkedIn', href: 'https://www.linkedin.com/company/hibit/', icon: (<svg fill="currentColor" viewBox="0 0 24 24" width="16" height="16"><path d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6zM2 9h4v12H2z" /><circle cx="4" cy="4" r="2" /></svg>) },
...
const titleStyle: React.CSSProperties = { color: 'var(--heading-color)', fontSize: 13, marginBottom: 12, fontWeight: 600 }
const listStyle: React.CSSProperties = { listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 9 }
const linkStyle: React.CSSProperties = { color: 'var(--text-secondary)', fontSize: 13, textDecoration: 'none' }

export default function Footer() {
  return (
    <footer style={{ background: 'var(--footer-bg)', borderTop: '1px solid var(--border-green)', userSelect: 'none' }}>
...
/>
                <span style={{ color: 'var(--heading-color)', fontWeight: 600 }}>Hibit</span>
              </Link>
            ...
            <div>
              ...
              <ul style={listStyle}>{quickLinks.map(l => (<li key={l.label}><Link href={l.href} style={linkStyle}>{l.label}</Link></li>))}</ul>
            </div>

            <div>
              <h4 style={titleStyle}>Services</h4>
              <ul style={listStyle}>{services.map(s => (<li key={s}><Link href="/ser" style={linkStyle}>{s}</Link></li>))}</ul>
            </div>

            <div>
              <h4 style={titleStyle}>Other</h4>
              <ul style={listStyle}>{otherServices.map(s => (<li key={s}><Link href="/ser" style={linkStyle}>{s}</Link></li>))}</ul>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <input placeholder="Email" style={{
                  ...
                  background: 'var(--bg-main)', border: '1px solid var(--border-green)',
                  color: 'var(--heading-color)', fontSize: 12, outline: 'none',
                }} />
                <button style={{
        ...
        <div className="footer-mobile">
          <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            ...
            <span style={{ color: 'var(--heading-color)', fontWeight: 600 }}>LOGO</span>
          </Link>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 24 }}>
            {socials.map(s => (
              <Link key={s.label} href={s.href} style={{
                ...
                background: 'var(--badge-bg)', border: '1px solid var(--border-green)',
                display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--green-lime)',
              }}>{s.icon}</Link>
            ))}
          </div>
          <h4 style={titleStyle}>Newsletter</h4>
        ...
        <div style={{
          ...
          fontSize: 12, color: 'var(--text-secondary)', flexWrap: 'wrap', gap: 10,
        }}>
