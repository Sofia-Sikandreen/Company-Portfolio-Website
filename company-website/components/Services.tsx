...
const services = [
  { title: 'Web Development', color: 'var(--text-primary)', icon: '💻' },
  { title: 'Laravel Dev', color: 'var(--text-primary)', icon: '⚙️' },
  { title: 'WordPress', color: 'var(--text-primary)', icon: '🌐' },
  { title: 'Shopify', color: 'var(--text-primary)', icon: '🛒' },
  { title: 'YouTube Automation', color: 'var(--text-primary)', icon: '▶️' },
  { title: 'Next.js Apps', color: 'var(--text-primary)', icon: '⚡' },
]
    ...
    <section id="services"      style={{background: "linear-gradient(135deg,var(--green-bright),var(--green-lime),var(--green-dark))" ,overflow: 'hidden', userSelect: "none", cursor: "default" }} className="services-section">
            ...
            {[...services, ...services].map((s, i) => (
              <Link key={i} href="/ser" style={{ textDecoration: 'none' }}>
                ...
                >
                  ...
                  <p style={{ color: 'var(--primary-text)', fontSize: 11, fontWeight: 600, margin: 0 }}>
                    {s.title}
                  </p>
                </div>
              </Link>
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          .services-section { padding: 32px 0 28px; margin-top: 16px; }
          .service-card { width: 110px; height: 65px; }
        }
      `}</style>
    </section>
  )
